...
function hsgxia2shxurxu(nstr,nwbk)
{
	...
	<body>
			<form action="addYonghuxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="20" colspan="2" background="images/table_header.gif">添加用户信息</td>
						</tr>
...
<script language=javascript >  
function hsgcheck() {
		var yonghucheng = $("#yonghucheng").val();
		if(yonghucheng==""||(yonghucheng.length<3||yonghucheng.length>12)){
			 $("#clabelyonghucheng").html("<font color=red>用户昵称不能为空且长度在3～12位之间！</font>");
			 $("input[id=yonghucheng]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelyonghucheng").html("");
			$.ajax({
				url : "quchongYonghuxinxi.do",
				type : "post",
				data : "yonghucheng=" + yonghucheng,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelyonghucheng").html("<font color=red>用户昵称已存在，请更换！</font>");
					$("input[id=yonghucheng]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelyonghucheng").html("系统异常，请检查错误！");
					$("input[id=yonghucheng]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
